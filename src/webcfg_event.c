	...
	while(FOREVER())
	{
		...
		if(eventDataQ != NULL)
		{
			...
			if(rv == WEBCFG_SUCCESS)
			{
				...
				if (((eventParam->status !=NULL)&&(strstr(eventParam->status, "ACK")!=NULL)) && (eventParam->timeout == 0))
				{
					//Based on ACK event if mesh/cujo is enabled then connected client notification need to be turned OFF
					if ((strcmp(eventParam->subdoc_name, "mesh") ==0) || (strcmp(eventParam->subdoc_name, "advsecurity") ==0))
					{
		...
		if( token != NULL )
		{
			...
			if(apply_status != NULL)
			{
				...
				if(attArr !=NULL)
				{
					...
					WebcfgInfo("Notify paramName %s\n", paramName);
					setAttributes(attArr, 1, NULL, &ret);
					if (ret != WDMP_SUCCESS)
					{
						WebcfgError("setAttributes failed for parameter : %s ret: %d\n", paramName, ret);
					}
					else
					{
						WebcfgInfo("setAttributes success for parameter : %s ret: %d\n",paramName, ret);
					}
